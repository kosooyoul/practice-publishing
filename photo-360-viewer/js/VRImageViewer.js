...
(function() {
	...
	clazz.constructor = function(element) {
		...
		this.prop = {
			...
			angleZ: Number(element.getAttribute('data-anglez')),
		...
		this.renderer.setSize(this.element.clientWidth, this.element.clientHeight); // Set the size of the WebGL viewport.
		...
		this.camera = new THREE.PerspectiveCamera(60, this.element.clientWidth / this.element.clientHeight, 0.1, 1000); // Define the perspective camera's attributes.
	...
	*/
	...
	var loadData = function(obj) {
		...
		if (isFinite(obj.prop.angleZ)) {
			obj.angle.z = obj.prop.angleZ;
			console.log('obj.angle.z', obj.angle.z);
		}
		...
		if (obj.src) {
			...
			obj.isLoading = true;
			obj.element.classList.add('loading');
			...
			var interval = setInterval(function() {
				...
				window.jsonp = getJsonpReceiver(obj);
	...
	var getJsonpReceiver = function(obj) {
		return function(jso) {
			script.remove && script.remove();
			script.removeNode && script.removeNode();
			window.script = null;
			window.jsonp = null;
			...
			initialize(obj);
			obj.isLoading = false;
			obj.element.classList.remove('loading');
	...
	var initialize = function(obj) {
		...
		obj.sphere.rotation.z = obj.angle.z * Math.PI / 180;
	...
	var initializeEvents = function(obj) {
		...
		var onMove = function(e) {
			...
			rx = obj.sphere.rotation.x - (pointer.pageY - startY) / obj.element.clientHeight * Math.PI;
			ry = obj.sphere.rotation.y - (pointer.pageX - startX) / obj.element.clientWidth * Math.PI;
		...
		window.addEventListener('resize', function(evt) { 
			//obj.renderer.setSize(evt.target.clientWidth, evt.target.clientHeight);

			//obj.camera.aspect = evt.target.clientWidth / evt.target.clientHeight;
			//obj.camera.updateProjectionMatrix();
		}, false);
